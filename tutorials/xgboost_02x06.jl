...
# ╔═╡ f95b3090-c23d-11ed-28d3-b1c62daaba1b
using DataFrames, CSV, MLJ, MLJXGBoostInterface, EvoTrees

# ╔═╡ 7c0eb7a4-2f0a-4c79-97bc-5ddbdac0c48d
begin
...
# ╔═╡ 88cb2212-e4a4-4b44-a286-2f79ca6a2123
dropmissing!(test_data, :SalePrice);

# ╔═╡ d34fb5d8-4c1d-428b-b033-f7fd2763e46e
y, X = unpack(test_data, ==(:SalePrice));

# ╔═╡ a5d227b5-25f8-4841-be58-ebcb037a5888
(X_train_full, X_valid_full), (y_train, y_valid) = MLJ.partition((X, y), 0.8, rng=123, multi=true);

# ╔═╡ 831ce2b2-581d-4522-8169-d22ea04291ec
X_train_full

# ╔═╡ fde31338-292e-44e8-b6ad-04ac1ced221d
low_cardinality_cols = [cname for cname in names(X_train_full) if length(unique(X_train_full[!,cname])) < 10 && 
                        eltype(X_train_full[!,cname]) <: AbstractString]

# ╔═╡ 547eaa17-d7e8-4dcf-a323-9eb97584a5da
numeric_cols = [cname for cname in names(X_train_full) if eltype(X_train_full[!,cname]) <: Number]

# ╔═╡ ffa3ddda-0673-44f5-a92e-46e385e61caa
my_cols = vcat(low_cardinality_cols, numeric_cols)

# ╔═╡ 2fd39172-0ca6-4e27-b0b5-69e482078dca
X_train = X_train_full[:,my_cols];

# ╔═╡ 1f9a0349-cc33-40a7-8db9-ed6fd6e1a260
X_valid = X_valid_full[:,my_cols];

# ╔═╡ 449719a3-3cbc-4838-80a2-5ee0c30a9fb4
coerce!(X_train, autotype(X_train))

# ╔═╡ 4a9e13b2-fd14-4e9c-aa81-3f42bf82e6cf
coerce!(X_valid, autotype(X_valid))

# ╔═╡ 4684bac8-1659-4baf-ab83-cc8f553959e2
begin
	hot = MLJ.OneHotEncoder()
	mach = MLJ.fit!(machine(hot, X_train))
	mach1 = MLJ.fit!(machine(hot, X_valid))
	X_train_hot = MLJ.transform(mach, X_train)
	X_valid_hot = MLJ.transform(mach1, X_valid)
end

# ╔═╡ ec919320-8fbd-4f89-a1a4-291f292a832e
EvoTreeRegressor = @load EvoTreeRegressor pkg=EvoTrees

# ╔═╡ 920e5a7d-2b4a-4aa2-b7fc-1c04c6791f34
model_1 = EvoTreeRegressor(rng=0)

# ╔═╡ c8f95aed-be8f-440f-8ecc-76ac6177f0f8
my_model_1 = XGBoostRegressor()

# ╔═╡ 48cacd00-0c64-444c-989b-4bdceb53eaeb
mach_1 = machine(model_1, X_train_hot, y_train)

# ╔═╡ d63d8983-ab96-4501-b68d-cdb0266990c0
MLJ.fit!(mach_1)

# ╔═╡ b46ac33a-8db7-40e2-94b3-dadd376db180
predictions_1 = predict(mach_1, X_valid_hot)

# ╔═╡ ba2eabe6-3f84-4881-87fa-759e1b7cc504
models(matching(X_train_hot,y_train)) |> DataFrame

# ╔═╡ c9ed350e-84e6-4c28-b46a-2202aa6341c1
mean_absolute_error(predictions_1, y_valid)

# ╔═╡ 7e786c45-83a4-4873-9ac5-e0755c227aa2
...
manifest_format = "2.0"
project_hash = "0ffc4946943015491bc832415e806ed2b7aab5c0"
...
git-tree-sha1 = "934d3f8887e420c1d5dcdf90f21e762c36195088"
