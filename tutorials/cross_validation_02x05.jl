...
# ╔═╡ 35baa510-c0c8-11ed-095e-19f47596c610
using DataFrames, CSV, MLJ, MLJDecisionTreeInterface

# ╔═╡ 5a5d0338-9a0d-4728-93e8-42ebeb0f7c78
using Plots
...
# ╔═╡ 94eedeb8-796b-4368-8425-9d6825750b09
numeric_cols = [cname for cname in propertynames(X_train_data) if eltype(X_train_data[:,cname]) <: Union{Number,Missing}]

# ╔═╡ 9cad958f-3569-44e6-8042-45f3229d7cc4
X = X_train_data[:, numeric_cols];

# ╔═╡ 8cb08fcc-6552-4800-bd3d-fe03e33e68ed
X_test = test_data[:, numeric_cols];

# ╔═╡ 87877301-8df3-4d14-9008-4b72a66069f4
preprocessor = FillImputer()

# ╔═╡ c9fa0ba1-f1d9-4cb2-aeba-8e95b1979250
Forest = @load RandomForestRegressor pkg=DecisionTree

# ╔═╡ 7b0f5976-37f2-46a1-8faa-3eedcd22db5f
model = Forest(n_trees=50, rng=0)

# ╔═╡ 8a9149d5-8a7a-4822-acec-5a70e9124944
my_pipeline = preprocessor |> model

# ╔═╡ c9a14c9d-705e-4e9d-8b2c-b9ad61a41a69
cv=CV(nfolds=5)

# ╔═╡ cb6a8705-f500-4e6b-99fb-26ca3f9de82e
scores = evaluate(my_pipeline, X, y, resampling=cv, measure=mae, verbosity=0)

# ╔═╡ 91d5250a-bbb3-44ac-86be-ed942358a955
println("Average MAE score: $(first(scores.measurement))")

# ╔═╡ 576cf711-f7d8-4968-bf68-889ab10929d9
...
# ╔═╡ 91ed94f0-b6aa-42fd-9bdf-3bb40701bf83
results = []

# ╔═╡ 829eae4e-543c-4918-9616-bebfb77faff0
for i in 1:9
	push!(results, get_score(50*i) => (50*i))
end

# ╔═╡ f951a901-6b0d-4524-907d-b6caef285c34
results

# ╔═╡ 68198a39-e52a-4267-8010-4c29ef9053c9
first.(results)

# ╔═╡ d17b0ffa-761a-44f7-816b-fb8274f5aeed
last.(results)

# ╔═╡ 92b9e100-8f88-4acc-ad1f-f58581c81514
plot(last.(results), first.(results))

# ╔═╡ 241ef243-a927-4b76-8d65-de50ea83e0b0
minimum(results)

# ╔═╡ 76157df1-8158-4ec3-b008-447f243fce53
...
manifest_format = "2.0"
project_hash = "7ff0e19bb51af9949a8b79dd5cac4a053cdba394"
...
git-tree-sha1 = "165be58f9fb7d84337bb377ecd01f20db3780545"
