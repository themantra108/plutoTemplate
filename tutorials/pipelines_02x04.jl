...
# ╔═╡ 9a394080-be5e-11ed-355d-c177d0247c32
using DataFrames, CSV, MLJ, CategoricalArrays, MLJDecisionTreeInterface

# ╔═╡ 86e9b229-ed78-4eaa-abca-0322d9946d5b
md"""
## Read the data
"""

# ╔═╡ d43dff0a-140c-4682-ad16-0e4f88ce9eb6
begin
	test_data = CSV.read("data\\home-data-for-ml-course\\train.csv", missingstring="NA", DataFrame)
	X_test_full = CSV.read("data\\home-data-for-ml-course\\test.csv", missingstring="NA", DataFrame)
end;

# ╔═╡ 7857ab34-8489-4069-a4fc-aef5271f63f5
coerce!(test_data, autotype(test_data));

# ╔═╡ b7bec572-4f17-4ce0-9b6f-14c94459f1c6
coerce!(X_test_full, autotype(X_test_full));

# ╔═╡ 49b91e8a-1413-475f-b648-00e747c0a049
y, X_full = unpack(test_data, ==(:SalePrice));

# ╔═╡ d9a1a3de-46a8-4feb-8932-720ac3b24e08
(X_train_full, X_valid_full), (y_train, y_valid) = partition((X_full, y), 0.8, rng=0, multi=true);

# ╔═╡ 8035bd00-4437-4f64-9dce-99ef4a94b361
categorical_cols = [cname for cname in propertynames(X_train_full) if
                    length(unique(X_train_full[:, cname])) < 10 &&
                    eltype(X_train_full[:, cname]) <: Union{CategoricalValue,Missing}];

# ╔═╡ 80fe2273-cb57-4235-bfe8-e2cc9443e4b8
numerical_cols = [cname for cname in propertynames(X_train_full) if 
                eltype(X_train_full[:, cname]) <: Union{Number,Missing}];

# ╔═╡ f3e35007-6a3f-49d1-a040-cec4f3d276c1
my_cols = vcat(categorical_cols, numerical_cols);

# ╔═╡ c37f6141-9c18-4a44-b259-60b06156e42b
X_train = X_train_full[:, my_cols];

# ╔═╡ ceb8f19f-c732-4c61-80ec-f9454a745a4e
X_valid = X_valid_full[:, my_cols];

# ╔═╡ a8964515-1d31-432a-bbe8-3f056d40330c
X_test = X_test_full[:, my_cols];

# ╔═╡ e0d01d86-9c31-44e8-bf18-0ed2cf53a569
propertynames(X_train)

# ╔═╡ 248b4e40-f8bb-44a3-bb93-64ba5ecb08d6
numerical_transformer = FillImputer()

# ╔═╡ ba2924db-5b7f-44e2-94e8-1b4bbd8fd3a8
mach1 = machine(numerical_transformer, X_train[:, numerical_cols]) |> fit!

# ╔═╡ b2d55eeb-fd21-4374-b871-67a2e60cfe12
num_X = MLJ.transform(mach1, X_train[:, numerical_cols])

# ╔═╡ e7a26615-d0fa-439e-910c-ab15c37c42c7
categorical_transformer = Pipeline(FillImputer(), OneHotEncoder())

# ╔═╡ df4bc94e-8f5d-4a98-a122-0d686403cb54
mach2 = machine(categorical_transformer, X_train[:, categorical_cols]) |> fit!

# ╔═╡ f57e13a7-985e-4c33-a8eb-73388764eaa4
cat_X = MLJ.transform(mach2, X_train[:, categorical_cols]);

# ╔═╡ 15414824-1249-4bb7-9b88-bfe2acb548b5
preprocessor = hcat(cat_X,num_X)

# ╔═╡ 902cbc6d-566a-479b-81f6-594613355cd3
model = RandomForestRegressor()

# ╔═╡ a14cee65-1c85-4cf9-ab49-ae734d1c1fb4
clg_mach = machine(model, preprocessor)

# ╔═╡ 74079ade-786c-4921-8837-ef96cc91e983
...
manifest_format = "2.0"
project_hash = "ea42173809df3480bf2f0f18e7819930a47e1103"
